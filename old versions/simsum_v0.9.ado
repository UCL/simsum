...
syntax varlist [if] [in], ///
    ...
    df(string) MODELSEMethod(string) ref(varname)                            /// calculation options
...
if "`long'"!="" {
    ...
    qui reshape wide `betalist' `se', i(`by' `id') j(`long') `string'
}
    ...
    foreach name in `output' {
        ...
        forvalues i=1/`m' {
            ...
            if `domcse' {
                ...
                local `name'_mcse ``name'_mcse' `name'_mcse_`i'
            }
        }
        ...
        if `domcse' {
            ...
            l `by' ``name'_mcse', noo subvarname `sepby' `sep'
        }
        if "`outsheet'"!="" {
            local `name'list
            forvalues i=1/`m' {
