...
syntax varlist [if] [in], ///
    [true(string) long(varname) id(varlist)                           /// input options
    SEPrefix(string) SESuffix(string) SEList(varlist)         /// input options
    level(real $S_level) max(real 10) semax(real 100) by(varlist) /// calculation options
    mcse df(string) MODELSEMethod(string) ref(varname)         /// calculation options
    bsims sesims bias empse relprec modelse relerror cover power  /// statistic options
    graph                                                         /// output options
    sepby(varlist) sep(passthru) format(string) /// listing options
    nolist oldout clear saving(string) gen(string)                 /// listing options
    listmiss nolistbig dropbig           /// handling awkward observations
    noMEMcheck]
...
if "`seprefix'"!="" | "`sesuffix'"!="" {
    if "`selist'"!="" {
        di as error "Can't specify selist() with seprefix() or sesuffix()"
        exit 498
    }
    forvalues i=1/`m' {
    ...
    local i 0
    foreach se of varlist `selist' {
        local ++i
        local se`i' `se'
    }
    if `i'<`m' {
        di as error "Fewer variables in selist(`selist') than in `betalist'"
        exit 498
    }
    if `i'>`m' {
        di as error "More variables in selist(`selist') than in `betalist'"
        exit 498
    }
}
...
if "`long'"!="" {
    ...
    qui reshape wide `betalist' `selist', i(`by' `id') j(`long') `string'
}
else forvalues i=1/`m' {
    local label`i' : var label `beta`i''
*    if "`label`i''"!="" local label`i' : subinstr local label`i' " method" ""
*    else local label`i' "`i'"
...
forvalues i=1/`m' {
    ...
    if "`selist'"!="" qui replace `missing' = 1 if missing(`se`i'') & `touse'
}
...
forvalues i=1/`m' {
    ...
    if `ninfb'+`ninfse' > 0 {
        ...
        if "`selist'"!="" di as text `"               "' as result `ninfse' as text `" observations with scaled `se`i'' >`semax'"'
        ...
        if "`dropbig'"!="dropbig" {
            ...
            if "`selist'"!="" di as error "Use semax() option to change acceptable limit of standard errors"
            exit 498
        }
        qui replace `beta`i'' = . if `infb'
        if "`selist'"!="" qui replace `se`i'' = . if `infse'
...
forvalues i=1/`m' {
    ...
    if "`cover'"=="cover" {
        qui gen cover_`i' = 100*(abs(`beta`i''-`truevar')<`crit'*`se`i'') if !missing(`beta`i'') &   !missing(`se`i'') 
        local collmean `collmean' cover_`i' 
    }
    if "`power'"=="power" {
        qui gen power_`i' = 100*(abs(`beta`i'')>=`crit'*`se`i'') if !missing(`beta`i'') & !missing(`se`i'') 
