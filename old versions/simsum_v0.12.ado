...
To do list:
    ...
    check for zero standard errors! (e.g. occurring when a variable is dropped from a regression)
*/
...
forvalues i=1/`m' {
    ...
    if `ninfb'+`ninfse' > 0 {
        ...
        if "`se`i''"!="" di as text `"               "' as result `ninfse' as text `" observations with scaled `se`i'' >`semax'"'
        ...
        if "`se`i''"!="" qui replace `se`i'' = . if `infse'
