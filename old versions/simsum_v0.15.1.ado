*! version 0.15.1  13apr2011 - new mse option
/***********************************************************************************************
HISTORY
* version 0.14.1 24aug2010 - better error handling if called with no observations
* version 0.14 7jun2010 - byvar.ado included as subroutine; changed to version 10; dropped separate() option and restored sep() abbreviation; incorporated listsepformat() into format(); added abbreviate(). For SJ resubmission.
...
syntax varlist [if] [in], ///
    ...
    bsims sesims bias empse relprec mse modelse relerror cover power         /// statistic options
    sepby(varlist) clear saving(string) ABbreviate(passthru)                 /// output options
    nolist listsep format(string) gen(string)                                /// output options
...
if "`bsims'`sesims'`bias'`empse'`relprec'`mse'`modelse'`relerror'`cover'`power'"=="" {
    foreach stat in bsims bias empse relprec mse {
        local `stat' `stat'
    }
    if "`se1'"!="" {
...
qui gen mcse = .
qui gen `gen'num = .
foreach stat in bsims sesims bias empse relprec modelse relerror cover power mse {
    local ++i
    ...
    * label statistics
    if "`stat'"=="bsims" local label "Non-missing point estimates"
    if "`stat'"=="sesims" local label "Non-missing standard errors"
    if "`stat'"=="bias" local label "Bias in point estimate"
    if "`stat'"=="empse" local label "Empirical standard error"
    if "`stat'"=="relprec" local label "% gain in precision relative to method `label`refmethod''"
    if "`stat'"=="mse" local label "Mean squared error"
    if "`stat'"=="modelse" {
    ...
    if "`stat'"=="relerror" local label "Relative % error in standard error"
    if "`stat'"=="cover" local label "Coverage of nominal `level'% confidence interval"
    if "`stat'"=="power" local label "Power of `alpha'% level test"
...
local pctfmt "`2'"
local nfmt "`3'"
if "`bfmt'"=="" local bfmt: format `beta1'
...
if "`list'"!="nolist" {
    if "`listsep'"=="" {
        ...
        foreach stat in `output' {
            ...
            local thisbetas = cond(inlist("`stat'","bsims","sesims"), "betasnomcse", "betas")
            ...
            list `by' ``thisbetas'' if `gen'code=="`stat'", noo subvarname sepby(`gen'num `sepby') `abbreviate'
        }
    }
}
